...
############################## NOTES ###################################
# Script Name: adobe_rum_swiftdialog_minimalist.sh
#
# Recompiled from various sources. Adam Duran, March 15, 2024
# Updated by Tony Young, April 25, 2025
# Finalized: Owain Iorwerth (MacAdmins: Poppy)
# Date: 12 May 2025
#
...
#
# Summary: Adobe updater for macOS using Adobe RUM and SwiftDialog with minimalist
# visual approach.
...
adobe_icon="'/Applications/Utilities/Adobe Creative Cloud/ACC/Creative Cloud.app/Contents/Resources/CreativeCloudApp.icns'"
...
checkForUpdates() {
    ...
    if ! grep -iq "updates are applicable on the system" "$rumlog"; then
        log "No Updates available."
        exit 0
    fi

    # If no Adobe apps are running, perform a silent update with notifications only
    if [[ "$adobeAppsRunning" = false ]]; then
        ...
        # Configure the SwiftDialog popup for user interaction
        dialogContent+=(
            --title none
            --moveable
            --width "450"
            ...
            --button1text "Quit and Update"
            --icon "$adobe_icon"
            --iconsize 80
        )

        # Display SwiftDialog popup and capture the user's response
        $dialogPath "${dialogContent[@]}"
        returncode=$?

        # Handle user interaction with the SwiftDialog popup
        case ${returncode} in
        0)
            log "User clicked Quit and Update. Proceeding..."
            installUpdates
            displaynotification "Wait for success message before opening" "Adobe Updates in Progress..."
            updatesComplete
            ;;
        *)
            log "Dialog dismissed or unexpected response: ${returncode}"
            exit 0
            ;;
        esac
    fi
